...
async def select_action(message: types.Message, state: FSMContext):
    ...
    cursor.execute("SELECT rdn1_id, rdn2_id, rdn3_id, blocked FROM access WHERE user_telegram_id = %s", (data['user_telegram_id'],))
    ...
    if message.text == "Заблокировать":
        ...
        await message.answer(f'ID {rdn1_id}, {rdn2_id}, {rdn3_id} для пользователя с user_telegram_id {data["user_telegram_id"]} заблокированы', reply_markup=mainmenu.main_kb(message.from_user.id))
        ...
        await message.answer(f'ID {rdn1_id}, {rdn2_id}, {rdn3_id} для пользователя с user_telegram_id {data["user_telegram_id"]} разблокированы', reply_markup=mainmenu.main_kb(message.from_user.id))
    await state.finish()
